"""Phase 13C validation script for TradeSense context engine."""
...
def main() -> None:
    ...
    context_engine = TradeSenseContextEngine()
    ...
    context_output = context_engine.build_context(
        prediction=prediction_output,
        decision=decision_output,
        features=feature_row.iloc[0].to_dict(),
    )

    missing = [field for field in _REQUIRED_CONTEXT_FIELDS if field not in context_output]
    if missing:
        raise RuntimeError(f"context output missing required fields: {missing}")
